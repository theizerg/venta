	...
	$('#btnBuscarCliente').on('click', function(e) {
		...
		$.get(url , function( data ){			    
			...
			if (proveedores.length < 1) {
				alert('Sin proveedores registrados')
			}
			$("#tablaClientes").html("");
			for(i=0; i < proveedores.length; i++){
				var cliente_id = proveedores[i]["id"];
				var cliente_nombre = proveedores[i]["nombre"];

				var proveedor_razon_social = "";
				if(proveedores[i]["razon_social"] != null){
					var proveedor_razon_social = proveedores[i]["razon_social"];	
				}

				var cliente_rut = "-";
				if(proveedores[i]["rut"] != null){
					var cliente_rut = proveedores[i]["rut"];	
				}

				var cliente_mail = "-";
				if(proveedores[i]["mail"] != null){
					var cliente_mail = proveedores[i]["mail"];	
				}

				var cliente_direccion = "-";
				if(proveedores[i]["direccion"] != null){
					var cliente_direccion = proveedores[i]["direccion"];	
				}
				...
				$("#tablaClientes").append(
					$('<tr></tr>').html(
						...
						+ "</td><td class='td_cliente_nombre'>"
							+ cliente_nombre + " " + proveedor_razon_social
